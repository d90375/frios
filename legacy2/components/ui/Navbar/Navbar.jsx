...
const Navbar = () => {
  ...
  const bgColor = addColor("transparent", "transparent");
  // Brand Color config
  const brandColor = addColor("text-gray-90", "text-white");
  ...
  return (
    <header>
      <nav
        className={`${bgColor} dark:bg-gray-900 w-full md:static md:text-sm ${state ? "relative z-20" : ""}`}
      >
        <div className="custom-screen relative items-center mx-auto md:flex">
          ...
          <div
            className={`${bgColor} dark:bg-gray-900 flex-1 md:py-0 md:block md:static md:z-0 ${state ? "absolute z-20 inset-x-0 px-4 py-6 w-full" : "hidden"}`}
          >
